...
RegisterNetEvent('esx_policejob:rldrag')
AddEventHandler('esx_policejob:rldrag', function(Dragger)
			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'elwhite')

						TriggerServerEvent('esx_service:disableService', 'elwhite')
...
function OpenPoliceActionsMenu()
	local elements = {
		{ unselectable = true,    icon = "fas fa-elwhite",                      title = "elwhite" },
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		...
		ESX.OpenContext("right", elements, function(menu, element)
			...
			if data.current.value then
				TriggerServerEvent('esx_policejob:confiscatePlayerItem', GetPlayerServerId(player), data.current
				.itemType, data.current.value, data.current.amount)
			...
			ESX.TriggerServerCallback('esx_policejob:getVehicleInfos', function(retrivedInfo)
				...
				if not retrivedInfo.owner then
					elements[#elements + 1] = {
						unselectable = true,
						icon = "fas fa-user",
						title = TranslateCap('owner_unknown')
					}
				else
					elements[#elements + 1] = {
						unselectable = true,
						icon = "fas fa-user",
						title = TranslateCap('owner', retrivedInfo.owner)
					}
				end
			...
			ESX.TriggerServerCallback('esx_policejob:buyWeapon', function(bought)
				if bought then
					if data.current.price > 0 then
						ESX.ShowNotification(TranslateCap('armory_bought', data.current.weaponLabel,
	...
	ESX.OpenContext("right", components, function(menu, element)
		...
		if data.current.hasComponent then
			...
			ESX.TriggerServerCallback('esx_policejob:buyWeapon', function(bought)
				if bought then
					if data.current.price > 0 then
						ESX.ShowNotification(TranslateCap('armory_bought', data.current.componentLabel,
							ESX.Math.GroupDigits(data.current.price)))
					end
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'elwhite' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_policejob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'elwhite' and IsPedOnFoot(playerPed) then
...
--[[
zakujhandcuffspolice

Dodane przez kali

]]--
RegisterNetEvent('esx_policejob:zakujhandcuffspolice')
AddEventHandler('esx_policejob:zakujhandcuffspolice', function()
	isHandcuffed = not isHandcuffed
	local playerPed = PlayerPedId()
...
RegisterNetEvent('esx_policejob:zakujhandcuffsrope')
AddEventHandler('esx_policejob:zakujhandcuffsrope', function()
	isHandcuffed = not isHandcuffed
	local playerPed = PlayerPedId()
...
RegisterNetEvent('esx_interact:startHarvestNerka')
AddEventHandler('esx_interact:startHarvestNerka', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getInventoryItem('spknife').count >= 1  then
		xPlayer.removeInventoryItem(spknife, 1)
		TriggerClientEvent('esx_interact:startHarvestNerka', target)
	else
		...
		if isHandcuffed and dragStatus.isDragged then
			...
			if DoesEntityExist(targetPed) and IsPedOnFoot(targetPed) and not IsPedDeadOrDying(targetPed, true) then
				if not wasDragged then
					AttachEntityToEntity(ESX.PlayerData.ped, targetPed, 11816, 0.54, 0.54, 0.0, 0.0, 0.0, 0.0, false,
...
AddEventHandler('esx_policejob:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'elwhite' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i = 1, #players, 1 do
				if players[i].job.name == 'elwhite' then
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_policejob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'elwhite')
...
if ESX.PlayerLoaded and ESX.PlayerData.job == 'elwhite' then
...
Citizen.CreateThread(function()
	exports.qtarget:AddBoxZone("dzwonek", vector3(441.7989, -982.0529, 30.67834 + 0.10), 1, 1, {
		name = "dzwonek",
		heading = 11.0,
		debugPoly = false,
		minZ = 30.77834,
...
Citizen.CreateThread(function()
	exports.qtarget:AddBoxZone("szafka", vector3(461.29, -1000.38, 30.69), 1, 3, {
		name = "szafka",
		heading = 0,
		--debugPoly = true,
		minZ = 28.29,
...
Citizen.CreateThread(function()
	exports.qtarget:AddBoxZone("bossmenu", vector3(461.55, -986.26, 30.73 - 1), 1, 1, {
		name = "bossmenu",
		heading = 0,
		--debugPoly = true,
		minZ = 27.53,
		...
		options = {
			{
				event = "krs:policesociety",
				icon = "fas fa-laptop",
				label = "Otwórz laptop",
				job = "elwhite"
			},
			{
				event = "easy-documents:OpenBadgesMenu",
				icon = "fas fa-laptop",
				label = "Otwórz menu odznak",
				job = "elwhite"
			},
		},
		distance = 3.5
...
zadzwoniles = false
AddEventHandler('krs:dzwonek', function()
	if not zadzwoniles then
		ESX.ShowNotification('Powiadomiono policje')
		TriggerServerEvent('krs:dzwonekpd')
		zadzwoniles = true
		Wait(10000)
		zadzwoniles = false
	else
...
AddEventHandler('krs:szafka', function()
	OpenCloakroomMenu()
end)

AddEventHandler('krs:policesociety', function()
	if ESX.PlayerData.job.name == 'elwhite' then
		TriggerEvent('esx_society:openBossMenu', 'elwhite', function(data, menu)
			menu.close()
		end, { wash = false })
	end
end)
